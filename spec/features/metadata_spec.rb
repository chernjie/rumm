...
describe "using the server metadata api" do
  context "with credentials are present" do
    ...
    context "when I create a metadatum on a given server" do
      When {VCR.use_cassette('metadata/create') {run "rumm create metadatum on server silly-saffron --key RackConnectLBPool --value POOL-some-pool"}}
      Then {all_stdout =~ /created metadatum/}
      And {last_exit_status.should eql 0}
    end
    context "when I update a metadatum on a given server" do
      When {VCR.use_cassette('metadata/update') {run "rumm update metadatum RackConnectLBPool on server silly-saffron --value POOL-other-pool"}}
    ...
    context "when I destroy a metadatum on a given server that exists" do
      When {VCR.use_cassette('metadata/destroy') {run "rumm destroy metadatum RackConnectLBPool on server silly-saffron"}}
